...
# autostart
exec_always --no-startup-id feh --bg-fill ~/Bilder/Wallpaper/7pm.png
...
# text rendering and scalability on retina/hidpi diqsplays (thanks to pango).
...
# start brave
bindsym $mod+Shift+w exec brave --password-store=basic

# start thunar (file explorer)
...
bindsym $mod+Shift+z		gaps outer current minus 5y
